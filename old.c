...
static int add_enrypted_flag(const char* path)
{
		    ...
		    struct fuse_file_info *fi)
{
	...
	if (is_encrypted(fpath))
	{
		...
		{
			fclose(in);
			printf("ERROR: Could not open temporary decryption file.\n");
			return -errno;
		}

		if (!do_crypt(in, tmp, 0, STATE_DATA->password))
		{
			fclose(in);
			fclose(tmp);
			printf("ERROR: do_crypt failed to decrypt a file.\n");
			return -errno;
		}
		...
		if (fd == -1)
		{
			printf("ERROR: Could not open temporary decryption file for readback.\n");
			remove(tmppath);
			return -errno;
		}
